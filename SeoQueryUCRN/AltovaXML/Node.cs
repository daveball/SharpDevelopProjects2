...
namespace Altova.Xml
{
	...
	public abstract class Node:IDisposable
	{
		...
		{
			domNode = doc.CreateRootElement(namespaceURI, name);
		...
		protected internal void DeclareNamespace(string prefix, string URI)
		{
			...
			if (attrs != null) 
			{
				for (int i = 0; i < attrs.Count; i++) 
				{
					...
					if (attr.Value == URI && attr.Name.StartsWith("xmlns")) // namespace URI already mapped?
		...
		public static string GetDomNodeValue(XmlNode node)
		{
			return node.InnerText;
		}

		protected static void SetDomNodeValue(XmlNode node, string v)
		{
			node.InnerText = v;
		}
		...
		public static bool DomElementNameEquals(XmlNode node, string URI, string name)
		{
			if (node.NodeType != XmlNodeType.Element)
				return false;
			if (name == null)
				return true;
			return (node.NamespaceURI == URI && node.LocalName == name);
		}
		...
		protected bool HasDomChild(NodeType type, string URI, string name)
		{
			...
			else
			{
				foreach (XmlNode node in domNode.ChildNodes)
					if (DomElementNameEquals(node, URI, name))
						return true;
				return false;
			}
		}
		...
		protected XmlNode GetDomNextChild( NodeType type, string URI, string name, XmlNode prevNode )
		{
			if( type == NodeType.Element && prevNode != null )
			{
				...
				while( temp != null )
				{
					if (DomElementNameEquals(temp, URI, name))
						return temp;
		...
		public static XmlNode dereference (XmlNode node)
		{
            ...
            string href = ((XmlElement)node).GetAttribute("href");
            if (href == null || href.Length == 0)
                return node;

            href = href.Substring(1);

            for (XmlNode refNode = node.OwnerDocument.DocumentElement.FirstChild.NextSibling; refNode != null; refNode = refNode.NextSibling)
                if (refNode.NodeType == XmlNodeType.Element && ((XmlElement)refNode).GetAttribute("id") == href)
                    return refNode;
